(fn [state time-left]
  ...
  (defn add-me
    ...
    (let [self (get-in-arena local-coords arena)]
      (assoc enriched-state :me (assoc :local-coords) (:contents self))))
...
(defn add-closest-food
  ...
  (defn add-shoot-path
    [{:keys [sorted-arena local-coords-arena] :as enriched-state}]
    (let [me (:me enriched-state)
          orientation (:orientation me)
          origin ()
          adj-fns {:e (fn [x y] [(inc x) y])
                   :w (fn [x y] [(dec x) y])
                   :n (fn [x y] [x (inc y)])
                   :s (fn [x y] [x (dec y)])}]
      )
